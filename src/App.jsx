    ...
    <div className="min-h-screen bg-gray-50 text-black">
      <div className="max-w-6xl mx-auto p-3 md:p-6">
          ...
          <div className="md:hidden divide-y">
            {JACKPOT_GAMES.map(g=>(
              <div key={g.id} className="p-3">
                <div className="flex justify-between mb-2"><span className="font-black text-xs bg-black text-white px-2 py-0.5 rounded">#{g.id}</span><span className="text-[10px] bg-yellow-400 px-2 py-0.5 rounded-full font-bold">{g.confidence} CONF</span></div>
                <p className="font-bold text-sm mb-2">{g.match}</p>
                <div className="grid grid-cols-4 gap-1.5 text-[11px]">
                  <div className="bg-blue-50 border border-blue-200 p-1.5 rounded text-center"><p className="text-[9px] text-gray-500">1X2</p><p className="font-black text-blue-700">{g.pred_1x2}</p></div>
                  <div className="bg-purple-50 border border-purple-200 p-1.5 rounded text-center"><p className="text-[9px] text-gray-500">DC</p><p className="font-black text-purple-700">{g.dc}</p></div>
                  <div className="bg-green-50 border border-green-200 p-1.5 rounded text-center"><p className="text-[9px] text-gray-500">GOALS</p><p className="font-bold text-green-700 text-[10px]">{g.goals}</p></div>
                  <div className="bg-orange-50 border border-orange-200 p-1.5 rounded text-center"><p className="text-[9px] text-gray-500">BTTS</p><p className="font-black text-orange-700">{g.btts}</p></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border-2 border-black rounded-xl p-4 mb-6">
          <h3 className="font-black text-red-600 underline mb-3 text-sm">PREMIUM SUBSCRIPTION PLANS - UNLOCK ALL COLUMNS</h3>
          <ul className="space-y-2 text-sm">
            <li>Pay <b className="text-blue-600">100/-</b> - Full Table + Over 1.5 Tips (12.00 odds)</li>
            <li>Pay <b className="text-red-600">350/-</b> - 6 Versions with DC & BTTS Analysis</li>
            <li>Pay <b className="text-red-600">250/-</b> - Weekend Pro with Confidence %</li>
            <li className="font-black pt-2 border-t mt-2"><span className="text-green-700">MPESA TILL: 1581046</span> <span className="text-red-600">(Buy goods)</span> | <span className="text-green-700">HELPLINE: 0793921100</span></li>
          </ul>
        </div>

        <div className="bg-gradient-to-r from-green-600 to-green-700 text-white p-5 rounded-xl text-center">
          <h3 className="font-black text-lg mb-1">🔓 GET FULL ANALYSIS - KES 350</h3>
          <p className="text-xs opacity-90 mb-3">Lipa Till 1581046 to unlock Over 1.5, DC, BTTS + 6 Versions</p>
          <a href="https://wa.me/254793921100?text=I%20paid%20KES%20350%20to%20Till%201581046%20for%20Full%20Predictions" className="inline-block bg-yellow-400 text-black px-8 py-3 rounded-full font-black">PAY & WHATSAPP</a>
        </div>
      </div>
    </div>
  )
